...
app.get('/test', async(req,resp)=>{
	...
	let data = await Promise.all(kijijiQuery.reduce(async (container,item)=>{
		...
		if(item['neighborhood'].length != 0 || item['stations'].length != 0){
			const data = Object.assign({},{lat:item['geo:lat'],long:item['geo:long'],price:item['g-core:price'] * Math.random()})
				let result = await insert(data).then(async data=>{
					let commuting = await transitTime([item['geo:lat'],item['geo:long']])
					container.push(Object.assign(item,{'commuting':commuting}))
				}).catch(e=>{
					console.log('Error:', e);
				})		
			}
